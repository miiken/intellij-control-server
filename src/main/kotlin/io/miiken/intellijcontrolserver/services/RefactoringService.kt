...
object RefactoringService {
    ...
    fun rename(
        ...
        try {
            ...
            val psiFile =
                PsiUtils.findPsiFile(project, request.filePath)
            ...
            ApplicationManager
                ...
                .executeOnPooledThread {
                    try {
                        ...
                        val references =
                            ApplicationManager.getApplication().runReadAction<List<com.intellij.psi.PsiReference>> {
                        ...
                        logger.info(
                            "Rename settings: isMethod=$isMethod, searchInComments=$searchInComments, searchInStrings=$searchInStrings",
                        )

                        // Create rename processor and find usages (can run in background)
                        val processor =
                            ApplicationManager.getApplication().runReadAction<RenameProcessor> {
                        ...
                        logger.info(
                            "Rename successful: ${request.oldName} -> ${request.newName}, $referencesCount changes in ${affectedFiles.size} file(s)",
                        )
